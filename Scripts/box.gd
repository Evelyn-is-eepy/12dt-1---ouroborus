...
func move_self(direction):
	...
	movable_objects_ray.target_position = direction * TILE_SIZE
	# Telling other boxes to move should be safe,
	# As this only happens once it's verified that they're not blocked.
	if movable_objects_ray.is_colliding():
		var object_to_move = movable_objects_ray.get_collider().get_parent()
...
func fall_into_hole() -> void:
	...
	falling_tween.set_ease(Tween.EASE_IN)
	falling_tween.set_trans(trans_type)
	falling_tween.tween_property(self, 'scale', Vector2(0,0), duration)
	falling_tween.tween_property(self, 'position', position + Vector2(0,8.0), duration)
	falling_tween.tween_property(self, 'rotation', PI/2,duration)
