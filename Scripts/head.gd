...
func _process(_delta: float) -> void:
	...
	if move_direction and can_move:
		...
		if not movement_obstructed and body_length < max_body_length:
			...
			moving = true
			tween.tween_property(self,'position',position + move_direction * TILE_SIZE,0.1).set_trans(Tween.TRANS_BOUNCE)
			...
			#quickly move head forward so code still works
			position += move_direction * TILE_SIZE
		
			#update snake body
			var previous_position = position - move_direction * TILE_SIZE
			var previous_cell = body_tilemap.local_to_map(previous_position)
			var new_tile_id = Vector2i(0,0)
			...
			#move head back so movement is still smooth
			position -= move_direction * TILE_SIZE
			...
			print('bonk!')
			#bonk animation not implemented...
...
func _on_area_2d_body_entered(body: Node2D) -> void:
	#logic for colliding with tile entities (e.g pits)
...
func _on_area_2d_area_entered(area: Area2D) -> void:
	...
	elif area.get_parent().is_in_group('crown'):
